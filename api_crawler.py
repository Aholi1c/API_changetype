...
TEMP_CSV = "api_crawl_temp.csv"       # 临时文件（用于断点续爬，避免数据丢失）

# 爬取策略配置（针对大数量URL优化）
MAX_RETRIES = 3                       # 每个URL最多重试3次（平衡效率与成功率）
RETRY_DELAY = 1                       # 重试间隔1秒（减少总耗时）
MAX_WORKERS = 8                       # 线程数：建议8-16（根据CPU/网络调整，避免被封IP）
BATCH_SIZE = 50                       # 每爬取50条URL，同步一次临时文件（防止崩溃丢失数据）
...
def crawl_single_api(url, original_row_num):
    ...
    for retry in range(MAX_RETRIES):
        try:
            visit_tool = Visit()  # 每个线程独立初始化，避免线程安全问题
            crawl_params = json.dumps({"url": url, "goal": "提取API版本更新内容"})
